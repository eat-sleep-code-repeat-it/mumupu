...
#
##***********************************************
## -m  from_key: music major of notes
## -k  to_key  : harmonica major you want to play the music
##      e.x. We have F major movable-do notes in zhipu format 
## -i  input file
## -a  adjust number of half-note, -/+ move down/up [ optional]
## -o  output file optional [ optional]
##      e.x. We want to get F major fix-do notes so we can use to play in a C major harmonica
##      python zhipu-transpose.py -m F -k C -i songs/sometime-when-it-rains.jps
##      python zhipu-transpose.py -m F -k C -a -12 -i songs/sometime-when-it-rains.jps
##
##      e.x. We have A major movable-do notes in zhipu format with a Ab major harmonica. 
##           We want to get Ab movable-do notes do we can use to play in a Ab major harmonica
##      python zhipu-transpose.py -m A -k Ab -i TomatoNotes/Ab.KissTheRain.jps
## 
## transpose from one key to another
##    e.x.  we want to move all notes up 5 half-note. the following two ways give you same result
##        python zhipu-transpose.py -a 5 -i songs/Londonderry.jps
##        python zhipu-transpose.py -m F -k C -i songs/Londonderry.jps
##      or one octave lower
##        python zhipu-transpose.py -a -7 -i songs/Londonderry.jps
##
...
def move_note(note, num_semitone):
  ...
  adjust = sharp - flat
  if (adjust < 0):
    if is_sharp_note == True:
      return sharp_notes[pos] + flat_symbol * abs(adjust)
    else:
      return flat_notes[pos] + flat_symbol * abs(adjust)
  else:
    if is_sharp_note == True:
      return sharp_notes[pos] + sharp_symbol * adjust
    else:
      return flat_notes[pos] + sharp_symbol * adjust
...
###########################################################
# True for G A D A B E
# False for F Bb Eb Ab
is_sharp_note = True
