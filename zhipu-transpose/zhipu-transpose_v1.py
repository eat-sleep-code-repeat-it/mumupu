...
def transpose_file(from_key, to_key, adjustment = 0, filename = '', tofilename =''):
  ...
  if is_valid_format(lines) == False:
    print("could not validate your file format:" + filename)
    return
  ...
  for line in lines:
    ...
    if count <= 8:
      if line.strip().startswith("D:"):
        resultlines += "D: " + fanQieKey(from_key) + "\n"
      elif line.strip().startswith("Z:"):
        resultlines += "Z: fixed-do\n"
      else:
        # skip 8 lines of header section
        resultlines += line
...
def main_with_args():
  ...
  if movable_key_v == None and key_of_harmonica_v == None:
  ...
  if is_valid_key(movable_key_v) and is_valid_key(key_of_harmonica_v):
